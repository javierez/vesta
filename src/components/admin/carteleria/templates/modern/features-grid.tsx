...
import type { TemplatePropertyData, TemplateConfiguration } from "~/types/template-data";
    ...
    <div>
      {config.showIcons ? (
        (() => {
          ...
          return (
            <div className={`${shouldCompact ? 'mt-3' : 'mt-4'} ${leftMargin}`}>
              <div className={`grid grid-cols-2 gap-x-5 ${shouldCompact ? 'gap-y-1.5' : 'gap-y-1'} max-w-fit`}>
                {features.map((feature, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <div className={cn(modernColors.text)} style={{ fontSize: shouldCompact ? "11px" : "12px" }}>
                        {feature.value}
                      </div>
                    </div>
                  );
        ...
        <div className={`${shouldCompact ? 'mt-3' : 'mt-4'} ${bulletMargin}`}>
          <ul className={shouldCompact ? "space-y-0.5" : "space-y-1"} style={{ fontSize: shouldCompact ? "11px" : "12px" }}>
            ...
            {config.additionalFields.slice(0, 3).map((fieldValue) => (
              <li key={fieldValue} className={cn("flex items-start", modernColors.text)}>
                <span className="mr-2 mt-0.5">•</span>
