        ...
        {AVAILABLE_FIELDS.map((field) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              className={cn(
                "flex flex-col items-center justify-center p-3 rounded-lg border-2 text-xs font-medium transition-all duration-200",
                "hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2",
                isSelected && "bg-blue-50 border-blue-500 text-blue-700",
                !isSelected && !isDisabled && "bg-white border-gray-200 text-gray-700 hover:border-gray-300",
                isDisabled && "bg-gray-50 border-gray-200 text-gray-400 cursor-not-allowed opacity-60"
              )}
              title={isDisabled ? "Máximo 2 campos seleccionados" : field.description}
            >
      ...
      {config.additionalFields.length === 2 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p className="text-sm text-blue-800">
            ✓ Límite alcanzado. Selecciona otro campo para reemplazar uno existente.
          </p>
        </div>
      )}
    </div>
