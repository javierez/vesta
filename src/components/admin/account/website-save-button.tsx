...
export function WebsiteSaveButton({
  ...
  if (state === "idle" || (!hasUnsavedChanges && !error && !isPending))
    return null;
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        state === "modified" && "bg-gray-800 text-white hover:bg-gray-900",
        state === "saving" && "bg-gray-600 text-white",
        state === "saved" && "bg-green-600 text-white hover:bg-green-700",
        state === "error" && "bg-red-600 text-white hover:bg-red-700",
        className,
      )}
    >
      ...
      {state === "modified" && <Save className="mr-2 h-4 w-4" />}
