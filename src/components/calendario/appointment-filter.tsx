...
export function AppointmentFilter({
  ...
  const FilterOption = ({
    ...
    >
      <div
        className={`flex h-4 w-4 items-center justify-center rounded border ${filters[category].includes(value) ? "border-primary bg-primary" : "border-input"}`}
      >
  ...
  const FilterCategory = ({
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <ChevronDown
          className={`h-4 w-4 text-muted-foreground transition-transform ${expandedCategories[category] ? "rotate-180 transform" : ""}`}
        />
      </div>
  ...
  return (
    <div className="flex flex-1 items-center space-x-2">
      <input
        ...
        className="w-full max-w-xs rounded border py-2 pl-3 pr-2"
        value={searchQuery}
