...
import type { OperationType, ListingTypeFilter, TableColumn } from "~/types/operations";
...
interface OperationsTableProps {
  ...
  data: any[]; // TODO: Replace with proper type when data structure is defined
  totalCount: number;
}
                ...
                <TableRow key={item.id || index} className="hover:bg-gray-50">
                  {columns.map((column) => (
                    <TableCell key={column.key} className={column.width}>
                      ...
                      {column.key === 'select' && (
                        <Checkbox
                          checked={selectedItems.has(item.id?.toString() || index.toString())}
                          onCheckedChange={(checked) => 
                            onItemSelect(item.id?.toString() || index.toString(), checked as boolean)
                          }
      ...
      {totalCount > 0 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-700">
            ...
            {Math.min(currentPage * 20, totalCount)} of {totalCount} {operationType.toLowerCase()}
          </div>
