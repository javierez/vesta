...
export default function KanbanBoard({
  ...
  const _cardIds = useMemo(() => {
    return columns.flatMap((column) =>
      column.items.map((item) => item.id.toString()),
    );
  }, [columns]);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (
      ...
      toast.error(
        `No se puede mover ${typeNames[operationType]} de ${activeCard.status} a ${destinationColumnId}`,
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        `${typeNames[operationType]} movido a ${destinationColumnId}`,
      );
    } catch (error) {
      // Rollback on error
  ...
  const moveCardBetweenColumns = (
    ...
    const cardIndex = sourceColumn.items.findIndex(
      (item) => item.id === cardId,
    );
