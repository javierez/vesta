...
import {
  ...
  type OCRResult,
} from "~/server/ocr/ocr-initial-form";
...
export default function OCRExample({
  ...
  const [extractedData, setExtractedData] =
    useState<ExtractedPropertyData | null>(null);
  ...
  const handleBackgroundProcess = () => {
    ...
    alert(
      "Document processing started in background. Results will be logged to console.",
    );
  };
  ...
  return (
    <div className="space-y-4">
      <Card className="p-6">
        <h3 className="mb-4 text-lg font-semibold">OCR Document Processing</h3>
            ...
            <div className="mt-6 space-y-4">
              ...
              {ocrResult.success && (
                <>
                    ...
                    <Card className="bg-green-50 p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        {extractedData.cadastralReference && (
                          <div>
                            <span className="font-medium">Cadastral Ref:</span>{" "}
                            {extractedData.cadastralReference}
                          </div>
                        )}
                      </div>
                  ...
                  {ocrResult.detectedFields &&
                    Object.keys(ocrResult.detectedFields).length > 0 && (
                      <Card className="bg-blue-50 p-4">
                        <h4 className="mb-2 font-medium text-blue-800">
                          Detected Form Fields
                        </h4>
                        <div className="space-y-1 text-sm">
                          {Object.entries(ocrResult.detectedFields).map(
                            ([key, field]) => (
                              <div key={key} className="flex justify-between">
                                <span className="font-medium">{key}:</span>
                                <span>
                                  {field.text} ({field.confidence.toFixed(1)}%)
                                </span>
                              </div>
                            ),
                          )}
                  ...
                  <details>
                    <summary className="cursor-pointer font-medium text-gray-700">
                      View Raw Extracted Text ({ocrResult.extractedText.length}{" "}
                      characters)
                    </summary>
                    <Card className="mt-2 bg-gray-50 p-4">
                      <pre className="max-h-60 overflow-y-auto whitespace-pre-wrap text-sm text-gray-700">
              ...
              {ocrResult.error && (
                <Card className="bg-red-50 p-4">
                  <p className="text-sm text-red-800">{ocrResult.error}</p>
                </Card>
              )}
            </div>
          )}
