...
export default function ProgressBar({
  ...
  const progressPercentage =
    ((actualCurrentStep + 1) / filteredSteps.length) * 100;

  // Debug logging
  if (propertyType && skippedSteps.length > 0) {
    console.log(
      `ProgressBar Debug - Property: ${propertyType}, Skipped: [${skippedSteps.join(", ")}], Current: ${currentStep}, Filtered: ${currentFilteredStep}, Total: ${filteredSteps.length}`,
    );
  }
  ...
  const handleStepClick = (stepIndex: number) => {
    ...
    if (
      originalStepIndex === currentFormStep + 1 ||
      originalStepIndex < currentFormStep
    ) {
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        style={{ minHeight: variant === "compact" ? 40 : 80 }}
      >
        {/* Progress Line */}
