...
import { appointments, contacts, listings, properties, users, documents } from "~/server/db/schema";
import { updateAppointment } from "~/server/queries/appointment";
import type { AppointmentWithDetails, VisitSignatureDocument } from "~/types/visits";
 ...
 */
export async function markAppointmentAsCompleted(appointmentId: bigint, notes?: string): Promise<boolean> {
  try {
 ...
 */
export async function hasVisitSignatures(appointmentId: bigint): Promise<boolean> {
  ...
  const hasAgentSignature = signatures.some(doc => doc.filename.includes('firma-agent'));
  const hasVisitorSignature = signatures.some(doc => doc.filename.includes('firma-visitor'));
 ...
 */
export async function getVisitSignatures(appointmentId: bigint): Promise<VisitSignatureDocument[]> {
  const { db } = await getSecureDb();
  ...
  return signatures.map(sig => ({
    ...sig,
    signatureType: sig.filename.includes('firma-agent') ? 'agent' as const : 'visitor' as const,
 ...
 */
export async function getCompletedVisitAppointments(userId?: string): Promise<AppointmentWithDetails[]> {
  const { db, accountId } = await getSecureDb();
